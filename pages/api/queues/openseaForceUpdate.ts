...
const OpenseaForceUpdate = Queue(
    ...
    async (job: Job) => {
        const { tokenId, attempt, contractAddress, newImageUrl } = job
        ...
        try {
            const openseaResult = await forceUpdateOpenSeaMetadata(contractAddress, tokenId)
            const originalImageURL = openseaResult.image_original_url

            // TODO: copy logic that checks all metadata, not just the image CID
            if (!(originalImageURL || '').includes(newImageCID)) {
                ...
                if (delay) {
                    ...
                    const jobData = await OpenseaForceUpdate.enqueue(
                        { tokenId, contractAddress, attempt: totalAttempts, newImageUrl },
                        { delay, id: contractAddress + tokenId, override: true },
                    )
...
export async function startOpenseaForceUpdateLoop(tokenId: string, contractAddress: string, newImageUrl: string) {
    const jobData = await OpenseaForceUpdate.enqueue(
        { tokenId, contractAddress, attempt: 1, newImageUrl },
